...
xpath_firstLevel_node_img_format = '//div//a[%d]/img'
xpath_firstLevel_node_a_format = '//div//a[%d]'
...
def getFirstLevelInfo(browser):
    url = 'https://integrity.clarivate.com/integrity/xmlxsl/pk_browse_index.xml_lov_body?p_type=H&p_subsystem=TAR&p_seleccio=GEN_CON_CONDITION&p_loc=p_val_TAR1&p_lletra=0-9&p_from=GEN_CON_CONDITION&p_first=yes#'
    browser.get(url)
    sleep(2)
    ...
    while 1:
        try:
            treeNodeimg = browser.find_element_by_xpath(xpath_firstLevel_node_img_format % (2 * i - 1))
        except:
            #找到html最后一个节点之后,再找不到类似节点,则退出循环
            i -= 1
            break
        aTreeNode = browser.find_element_by_xpath(xpath_firstLevel_node_a_format % (2 * i - 1))
        aStrValue = browser.find_element_by_xpath(xpath_firstLevel_node_a_format % (2 * i))
        node = TreeNode(aTreeNode.get_attribute('href'), aStrValue.text,  0, 0)
        node.print()
...
def getFollowingLevelInfo(browser, parentNode):
    ...
    while 1:
        try:
            treeNodeimg = browser.find_element_by_xpath(xpath_sub_node_img_format % (1 + 2* i))
        except:
            #找到html最后一个节点之后,再找不到类似节点,则退出循环
            i -= 1

            break
        aTreeNode = browser.find_element_by_xpath(xpath_sub_a_format % (1 + 2* i))
        aStrValue = browser.find_element_by_xpath(xpath_sub_a_format % (2 + 2 * i))
...
if __name__ == "__main__":
    ...
    if len(nodeDict) != 0:
        filePath = '/home/ubuntu/GHDDI/download/integrity/target/condition_tree.csv'
