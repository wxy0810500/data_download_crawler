...
def doFetchData(year, yRange, condition, browser):
    ...
    ck1 = {'name': 'igr_user', 'value': '1403791688M460140%|ITGR|', 'domain': '.clarivate.com',
    ...
    sleep(1)
    # 点击Experimental Activity
    browser.find_element_by_id('EXP_EXPERIMENTAL_ACTIVITY').click()
    sleep(1)
    # 根据参数的elementId选择<td>元素,并点击其下一级别的a元素
    browser.find_element_by_xpath(r"//td[@id='{}']//a".format(condition["elementId"])).click()
    sleep(1)
...
def fetchData(year, yRange, condition):
    print("start :" + yRange)
    downloadFilePath = '/GHDDI/download/integrity/experimental-pharmacology/{}/{}'.format(condition.get('name'),  year)
...
conditions = [
    {"key":1, "name": "Toxicity", "elementId": "EXP_EA_TOXICITY", "cond": '"Toxicity/adverse events"'},# 相当于experimental activity 选择toxicity
    {"key":2, "name": "Pharmacological", "elementId": "EXP_EA_TARGET","cond": '"Pharmacological activities"'}, # 相当于experimental activity target
    {"key":3, "name": "therapeutic-no-infections", "elementId": "EXP_EA_CONDITION",
    "cond": '''"AIDS" or "Anesthesia" or "Cancer" or "Cardiovascular Disorders" or "Congenital defects" 
    ...
    or "Surgical and Medical Procedures"'''},# 相当于experimental activity 选择conditions
    {"key" : 4, "name":"condtion-infections", "elementId": "EXP_EA_CONDITION","cond": '"Infections"'}
]
...
if __name__ == "__main__":
    ...
    for _year in years:
        ...
        for _range in season_ranges:
            ...
            fetchData(str(_year), yRange, conditions[3])
            sleep(10)
