...
class TreeNode:
    ...
    def outputCsv(self):
        return '{};{};{};{};{}\n'.format(self.id, self.name,self.pid,  self.deep, self.sonNum)
...
# 初始化根节点
rootNode = TreeNode('https://integrity.clarivate.com/integrity/xmlxsl/pk_hierar_tree.hierarchyTreeEATarget?p_parent_id=0&p_action=click&p_element=p_val_EXP1#sn', 'PHARMACOLOGICAL ACTIVITIES', 0, 0)
...
if __name__ == "__main__":
    ...
    if len(nodeDict) != 0:
        filePath = '/home/ubuntu/GHDDI/download/integrity/expPharmacologicalActivitesTargetTree.csv'
        if os.path.exists(filePath):
            os.remove(filePath)
        outputFile = open(filePath, 'x')
        for node in nodeDict:
            outputFile.write(node.print())
    browser.quit()
